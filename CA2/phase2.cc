/*
 * phase2.cc
 * Phase 2: Wi-Fi 6 (802.11ax) simulation with OFDMA and MU-MIMO.
...
int main(int argc, char *argv[]) {
    ...
    phyHelper.Set("RxNoiseFigure", DoubleValue(rxNoiseFigure)); // Noise figure (dB)
    phyHelper.SetChannel(channelHelper.Create());

    // 2.3 Create Wi-Fi helper and set standard to 802.11ax
    WifiHelper wifiHelper;
    wifiHelper.SetStandard(WIFI_STANDARD_80211ax);

    // 2.4 Set rate control algorithm (constant rate) with HE MCS
    ...
    // Note: In ns-3.38, explicit OFDMA configuration is not available.
    // The simulation runs with 802.11ax standard and HE MCS rates.
